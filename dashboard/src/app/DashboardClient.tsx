...
export default function DashboardClient({ initialData }: { initialData: TasksFile | null }) {
  // Seed state from server-rendered data so first paint shows content immediately.
  const [data, setData] = useState<TasksFile | null>(initialData);
  ...
  // Fix 4: onUpdate applies the server-confirmed change directly — no extra fetchTasks()
  // call that could overwrite the new state with a stale GitHub read.
  ...
  // Refresh on mount to hydrate past the ISR cache window.
